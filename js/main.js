...
import { computeKpis, getDateRange, renderKpiSection } from './kpi.js';
...
async function loadInitialData() {
  ...
  if (published) {
    ...
    state.skuInfo = published.skuInfo || {};
...
function handleFileUpload(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      const { nvRecords, contHodo, loadedAt, skuInfo } = loadWorkbookFromArrayBuffer(data);
      ...
      downloadDataJson({ nvRecords, contHodo, loadedAt, skuInfo });
...
// ---------------------------------------------------------------------------
// Menú único, reducido a las 2 secciones de negocio reales: NV activas (lo
// único construido hoy) y NV concluidas (placeholder, aún no existe). El
// resto de las secciones de KPI y "Detalle" siguen existiendo en el código
// pero ya no aparecen en este menú — se retoman más adelante.
// ---------------------------------------------------------------------------
...
function buildMainMenu() {
  ...
  const items = [
    { kind: 'kpi', id: 'notasPendientes', label: 'KPI de Notas de Venta activas' },
    { kind: 'kpi', id: 'notasConcluidas', label: 'KPI de Notas de Venta concluidas' },
  ];
...
function renderKpiTab() {
  ...
  renderKpiSection(state.activeKpiSection, kpis, state.nvRecords, state.skuInfo);
}
