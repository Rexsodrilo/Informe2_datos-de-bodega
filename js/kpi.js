...
function renderNotasPendientes(el, nvRecords) {
  ...
  function renderFillRatePanel(universo, pool) {
    ...
    if (!cont) return;
    const entregada100Set = new Set(pool.entregada100);
    const noEntregada100 = universo.filter((nv) => !entregada100Set.has(nv));
    const montoEntregada100 = pool.entregada100.reduce((s, nv) => s + (nv.montoTotal || 0), 0);
    const montoNoEntregada100 = noEntregada100.reduce((s, nv) => s + (nv.montoTotal || 0), 0);
    cont.innerHTML = `
      ...
      <div class="np-zona4-grid">
        <div class="kpi-card kpi-card--success">
          ...
          <div class="kpi-card-value">${pool.entregada100.length}</div>
        </div>
        <div class="kpi-card kpi-card--success">
          <div class="kpi-card-label">Atendidas 100% · Dinero</div>
          <div class="kpi-card-value">${fmtMoney(montoEntregada100)}</div>
        </div>
        <div class="kpi-card kpi-card--danger">
          <div class="kpi-card-label">NO atendidas 100% · NV</div>
          <div class="kpi-card-value">${noEntregada100.length}</div>
        </div>
        <div class="kpi-card kpi-card--danger">
          <div class="kpi-card-label">NO atendidas 100% · Dinero</div>
          <div class="kpi-card-value">${fmtMoney(montoNoEntregada100)}</div>
        </div>
      </div>
  ...
  function renderDetalleCompartido(universo, pool) {
    ...
    if (detalleActivo.tipo === 'zona1') {
      ...
      list = listasPorId[detalleActivo.id];
      titulo = titulosPorId[detalleActivo.id];
      exportName = `nv_${detalleActivo.id}`;
    } else {
      list = computeAlertas(universo).bloqueadas;
      titulo = 'Bloqueadas';
      exportName = 'nv_bloqueadas';
    }

    cont.innerHTML = `
      <h4>Detalle — ${titulo}${list.length > 300 ? ` <span class="kpi-note">(mostrando las primeras 300 de ${list.length})</span>` : ''}</h4>
    ...
    document.getElementById('np-detalle-export')?.addEventListener('click', (e) => {
      downloadXlsx(
        `${exportName}_${new Date().toISOString().slice(0, 10)}.xlsx`,
