...
function renderNotasPendientes(el, nvRecords) {
  ...
  // piden ese producto), no por línea — así no se cuenta el mismo faltante
  // varias veces cuando distintas NV comparten el mismo producto agotado.
  function computeQuiebreStock(universo, conQuiebre) {
    ...
    conQuiebre.forEach((nv) => {
      nv.items.forEach((item) => {
        if (!item.quiebre || skusVistos.has(item.codProd)) return;
        skusVistos.add(item.codProd);
        const deficit = Math.max(0, (item.demandaVigente || 0) - (item.stockSistema || 0));
        const pctSku = item.demandaVigente > 0 ? (deficit / item.demandaVigente) * 100 : 0;
  ...
  function computeQuiebreDetalle(conQuiebre) {
    return conQuiebre.map((nv) => {
      const productosTexto = nv.items
        ...
        .map((it) => `${it.detProd || it.codProd || '(sin código)'} (faltan ${Math.max(0, (it.demandaVigente || 0) - (it.stockSistema || 0))})`)
        .join(' | ');
