...
function renderNotasPendientes(el, nvRecords) {
  ...
  let kpiDetalleAbierto = null; // 'facturar' | 'enProceso' | null — detalle expandible bajo las tarjetas de Estados
  ...
  function renderKpisPrincipales(universo, pool) {
    ...
    document.getElementById('np-kpis').innerHTML = `
      <div class="kpi-card-row">
        ...
        <div class="kpi-card">
          <div class="kpi-card-label">Sin entregar</div>
          ...
          <div class="kpi-card-subline">${pool.enProceso.length} en proceso</div>
    ...
    renderKpisDetalle(pool);
  }

  function renderKpisDetalle(pool) {
    ...
    const list = kpiDetalleAbierto === 'facturar' ? pool.pendientesFacturar : pool.enProceso;
    const titulo = kpiDetalleAbierto === 'facturar' ? 'Pendientes de facturar' : 'En proceso';
    cont.innerHTML = `
      <h4>${titulo}</h4>
  ...
  function renderFillRatePanel(universo, pool) {
    ...
    cont.innerHTML = `
      <div class="np-fillrate-grid">
        <div class="np-fillrate-item">
          <span class="np-fillrate-value">${universo.length}</span>
          <span class="np-fillrate-label">Total NV</span>
        </div>
        <div class="np-fillrate-item">
          <span class="np-fillrate-value">${atendidas}</span>
          <span class="np-fillrate-label">Atendidas en el período</span>
        </div>
        <div class="np-fillrate-item np-fillrate-item--success">
          <span class="np-fillrate-value">${pool.entregada100.length}</span>
          <span class="np-fillrate-label">Al 100%</span>
        </div>
        <div class="np-fillrate-item np-fillrate-item--warning">
          <span class="np-fillrate-value">${conQuiebreOParcial.length}</span>
          <span class="np-fillrate-label">Con quiebre o parcial</span>
        </div>
        <div class="np-fillrate-item np-fillrate-item--danger">
          <span class="np-fillrate-value">${pool.sinAccion.length}</span>
          <span class="np-fillrate-label">Sin atención del período</span>
        </div>
      </div>
