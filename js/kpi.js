...
function renderNotasPendientes(el, nvRecords) {
  ...
  el.innerHTML = `
    ...
    <div class="np-section-label">Notas de venta coordinadas — semana vigente</div>
  ...
  function computePool(universo) {
    ...
    // "Notas de venta en proceso de preparación": OP_Picking (Corte) ya
    // asignado, pero Status des y Trans (ambos de Corte) todavía vacíos —
    // el picking arrancó pero aún no se le asigna transporte.
    ...
    // todavía no la haya pasado a estado C (útil para detectar NV que
    // deberían cerrarse y se quedaron pegadas en A).
    const entregadas = universo.filter((nv) => (nv.guia || nv.factura) && nv.statusDes === 'DESPACHADO');

    // --- Fill Rate: mide cumplimiento de ítems (Cum_INV), una dimensión
    // independiente del flujo de picking/despacho de arriba ---
    const esCompleto = (nv) => nv.items.length > 0 && nv.items.every((it) => (it.inFull || 0) >= 0.999);
    const entregada100 = universo.filter(esCompleto);
    const sinAccion = universo.filter((nv) => !nv.statusDes);

    return { enProcesoPreparacion, enDespacho, entregadas, entregada100, sinAccion };
  }

  function renderKpisPrincipales(universo, pool) {
    const montoDe = (list) => list.reduce((s, nv) => s + (nv.montoTotal || 0), 0);
    const cardHtml = (id, label, list, toggleLabel) => `
      <div class="kpi-card">
        <div class="kpi-card-label">${label}</div>
        <div class="kpi-card-value">${list.length}</div>
        <div class="kpi-card-sub">${fmtMoney(montoDe(list))}</div>
        <button class="btn-export kpi-card-action" id="np-kpi-${id}-toggle">${kpiDetalleAbierto === id ? '▴ Ocultar' : `▾ ${toggleLabel}`}</button>
      </div>`;
    document.getElementById('np-kpis').innerHTML = `
      <div class="kpi-card-row">
        ${cardHtml('total', 'Total de notas de venta Aceptadas', universo, 'Ver detalle')}
        ${cardHtml('preparacion', 'Notas de venta en proceso de preparación', pool.enProcesoPreparacion, 'Ver detalle')}
        ${cardHtml('despacho', 'Notas de venta en despacho', pool.enDespacho, 'Ver detalle')}
        ${cardHtml('entregadas', 'Notas de venta entregadas', pool.entregadas, 'Ver detalle')}
      </div>
    `;

    ['total', 'preparacion', 'despacho', 'entregadas'].forEach((id) => {
      document.getElementById(`np-kpi-${id}-toggle`)?.addEventListener('click', () => {
        kpiDetalleAbierto = kpiDetalleAbierto === id ? null : id;
  ...
  function renderKpisDetalle(universo, pool) {
    ...
    const listasPorId = {
      total: universo,
      preparacion: pool.enProcesoPreparacion,
      despacho: pool.enDespacho,
      entregadas: pool.entregadas,
    };
    const titulosPorId = {
      total: 'Total de notas de venta Aceptadas',
      preparacion: 'Notas de venta en proceso de preparación',
      despacho: 'Notas de venta en despacho',
      entregadas: 'Notas de venta entregadas',
    };
    const list = listasPorId[kpiDetalleAbierto];
    const titulo = titulosPorId[kpiDetalleAbierto];
    cont.innerHTML = `
  ...
  // propósito no depende de los filtros de fecha/entrega/búsqueda de arriba,
  // igual que la pizarra física no cambia según lo que alguien esté filtrando.
  function computeSemanaVigente() {
    ...
    const dias = [];
    for (let i = 0; i < 6; i++) {
      dias.push({ fecha: new Date(lunes.getTime() + i * 86400000), nvs: [] });
    }
    nvRecords.forEach((nv) => {
  ...
  function renderSemanaPanel() {
    ...
    cont.innerHTML = dias
      .map(
        (d, i) => `
      <div class="np-semana-dia np-semana-dia--${i % 6}">
        <div class="np-semana-dia-header">${nombresDia[i]}<br>${d.fecha.toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit', year: '2-digit' })}</div>
        <div class="np-semana-dia-body">
          ${
            d.nvs.length === 0
              ? '<span class="np-semana-vacio">Sin NV coordinadas</span>'
              : d.nvs.map((nv) => `<div class="np-semana-nv" title="${nv.cliente || ''}">${nv.nvNumero}</div>`).join('')
          }
        </div>
      </div>`
      )
