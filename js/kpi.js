...
function renderNotasPendientes(el, nvRecords) {
  ...
  // línea es la real de esa línea (nvCant/nvTotLinea), no un total agregado
  // por producto: sumar esos agregados aquí los inflaba muy por encima del
  // valor real de cada NV (el mismo total del producto se contaba en cada
  // NV que lo pedía). El flag `quiebre` en sí sigue viniendo de comparar la
  // demanda total del producto contra el stock del sistema (dataLoader.js);
  // esto solo decide qué se MUESTRA para cada línea ya marcada.
  function computeQuiebreStock(universo, conQuiebre) {
    ...
    conQuiebre.forEach((nv) => {
      nv.items.forEach((item) => {
        if (!item.quiebre) return;
        totalUnidades += item.cant || 0;
        totalDinero += item.montoLinea || 0;
      });
    });
    const unidadesTotalUniverso = universo.reduce((s, nv) => s + nv.items.reduce((s2, it) => s2 + (it.cant || 0), 0), 0);
  ...
  // "Exportar productos con quiebre (Comex)". Cantidad y valor son los
  // reales de esa línea (nvCant/nvTotLinea).
