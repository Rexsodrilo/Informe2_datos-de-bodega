...
export function renderKpiSection(sectionId, kpis, nvRecords, skuInfo) {
  const el = document.getElementById('kpi-panel');
  if (!el) return;

  switch (sectionId) {
    case 'notasPendientes':
      renderNotasPendientes(el, nvRecords, skuInfo || {});
      break;
    case 'cumplimiento':
...
function renderNotasPendientes(el, nvRecords, skuInfo) {
  ...
  // El déficit se calcula UNA VEZ POR SKU (demandaVigente - stockSistema,
  // ambos ya calculados en dataLoader.js sumando TODAS las NV vigentes que
  // piden ese producto, y guardados en el "skuInfo" compartido, no por
  // línea) — así no se cuenta el mismo faltante varias veces cuando
  // distintas NV comparten el mismo producto agotado.
  function computeQuiebreStock(universo, conQuiebre) {
    ...
    conQuiebre.forEach((nv) => {
      nv.items.forEach((item) => {
        const key = skuKey(item.codProd);
        if (!item.quiebre || skusVistos.has(key)) return;
        skusVistos.add(key);
        const info = skuInfo[key];
        if (!info) return;
        const deficit = Math.max(0, (info.demandaVigente || 0) - (info.stockSistema || 0));
        const pctSku = info.demandaVigente > 0 ? (deficit / info.demandaVigente) * 100 : 0;
        totalDeficitUnidades += deficit;
        totalDineroPerdido += deficit * (info.precioPromedio || 0);
        sumaPctSku += pctSku;
        countSkus += 1;
      });
    });
    ...
    return {
      ...
      totalDineroPerdido,
      pctPromedioDeficit: countSkus > 0 ? sumaPctSku / countSkus : 0,
  ...
  // "Exportar productos con quiebre (Comex)". La cantidad/valor faltante es
  // a nivel de SKU (ver computeQuiebreStock): el mismo producto puede
  // repetirse en varias NV con el mismo déficit, porque compiten por el
  // mismo stock limitado.
  function computeQuiebreDetalle(conQuiebre) {
    ...
    conQuiebre.forEach((nv) => {
      nv.items
        ...
        .forEach((it) => {
          const info = skuInfo[skuKey(it.codProd)];
          const cantFaltante = info ? Math.max(0, (info.demandaVigente || 0) - (info.stockSistema || 0)) : 0;
          const valorFaltante = info ? cantFaltante * (info.precioPromedio || 0) : 0;
          filas.push({
  ...
  function renderDetalleCompartido(universo, pool) {
    ...
    if (detalleActivo.tipo === 'zona1') {
      ...
      headers = ['NV', 'Cliente', 'Código', 'Producto', 'Cantidad faltante', 'Valor'];
      rows = filas
        .slice(0, 300)
        .map((f) => [f.nvNumero, f.cliente, f.codProd, f.detProd, f.cantFaltante, fmtMoney(f.valorFaltante)]);
      xlsxHeaders = ['NV', 'Cliente', 'Código', 'Producto', 'Cantidad faltante', 'Valor'];
      xlsxRows = filas.map((f) => [f.nvNumero, f.cliente, f.codProd, f.detProd, f.cantFaltante, Math.round(f.valorFaltante)]);
    } else {
