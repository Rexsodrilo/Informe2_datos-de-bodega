...
function renderAlertas(el, data) {
  ...
  const tipos = distinct(data.noDespachadasAtrasadas.map((nv) => nv.tipoMovimiento));
  ...
  el.innerHTML = `
    <div class="kpi-alertas-header">
      ...
      <div class="kpi-alertas-filters">
        <select id="alertas-filtro-canal" class="kpi-filter-select">${filterOptions(canales)}</select>
        <select id="alertas-filtro-vendedor" class="kpi-filter-select">${filterOptions(vendedores)}</select>
        <select id="alertas-filtro-tipo" class="kpi-filter-select">${filterOptions(tipos)}</select>
      </div>
    </div>
  ...
  // Tipo, sin tocar el histórico ni "Despachado por canal" (que siguen
  // mostrando el universo completo, según lo acordado).
  let filtradas = data.noDespachadasAtrasadas;

  function renderBloqueA() {
    const canal = document.getElementById('alertas-filtro-canal').value;
    const vendedor = document.getElementById('alertas-filtro-vendedor').value;
    const tipo = document.getElementById('alertas-filtro-tipo').value;

    filtradas = data.noDespachadasAtrasadas.filter(
      (nv) =>
        (canal === 'todos' || nv.canal === canal) &&
        (vendedor === 'todos' || nv.vendedor === vendedor) &&
        (tipo === 'todos' || nv.tipoMovimiento === tipo)
    );
  ...
  renderBloqueA();
  ['alertas-filtro-canal', 'alertas-filtro-vendedor', 'alertas-filtro-tipo'].forEach((id) => {
    document.getElementById(id)?.addEventListener('change', renderBloqueA);
  });
}
