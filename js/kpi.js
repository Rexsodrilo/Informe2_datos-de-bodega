...
const NP_EXPLICACIONES = {
  despachado: 'NV en estado A con Status "DESPACHADO" y el 100% de sus productos entregados (Cum_INV completo). Ya salieron de bodega, solo falta que el sistema las cierre.',
  parcial: 'NV en estado A con Status "ENT_PARCIAL": se entregó parte del pedido, todavía queda saldo por despachar.',
  vacio: 'NV en estado A cuyo Status está vacío: aún no se ha entregado nada de este pedido.',
};

function statusBadgeHtml(nv) {
  if (nv.statusDes === 'DESPACHADO') return '<span class="status-badge status-badge--success">Despachado</span>';
  if (nv.statusDes === 'ENT_PARCIAL') return '<span class="status-badge status-badge--warning">Parcial</span>';
  if (!nv.statusDes) return '<span class="status-badge status-badge--danger">Sin entregar</span>';
  return `<span class="status-badge status-badge--neutral">${nv.statusDes}</span>`;
}
...
function renderNotasPendientes(el, nvRecords) {
  ...
  let fechaModo = 'semana'; // 'dia' | 'semana' | 'mes' | 'rango'
  ...
  let tablaAbierta = false; // la tabla de detalle parte cerrada, para no enterrar el análisis de abajo
  let nvSeleccionada = null; // NV con el detalle lateral abierto en la tabla
  ...
  el.innerHTML = `
    ...
    <div class="np-section-label">Filtros</div>
    <div class="np-filtros">
      ...
      <div class="kpi-filter-group np-search-group">
        <span class="kpi-filter-label">Buscar</span>
      ...
      <div class="kpi-filter-group">
        <span class="kpi-filter-label">&nbsp;</span>
        <button id="np-limpiar-filtros" class="btn-clear-filters">Limpiar filtros</button>
      </div>
    </div>

    <div class="np-section-label">Análisis</div>
    <h3>Despachos por día</h3>
    <div class="chart-box" id="np-chart-dia-box"><canvas id="chart-np-por-dia"></canvas></div>
    ...
    <div class="np-section-label">Notas de venta</div>
    <div id="np-tabla"></div>
    <div id="np-tabla-detalle-panel"></div>
  ...
  function computeUniverso() {
    const range = fechaModo === 'rango' ? fechaCustomRange || getFechaBounds() : getDateRange(fechaRefDate, fechaModo);
  ...
  function renderKpisPrincipales(universo, pool) {
    ...
    document.getElementById('np-kpis').innerHTML = `
      <div class="kpi-card-row">
        ${kpiCard('Total pendientes', universo.length, fmtMoney(montoDe(universo)))}
        ${kpiCard('Entregadas (pend. rebaja)', pool.despachado.length, fmtMoney(montoDe(pool.despachado)))}
        ${kpiCard('Entrega parcial', pool.parcial.length, fmtMoney(montoDe(pool.parcial)))}
        ${kpiCard('Sin entregar', pool.vacio.length, fmtMoney(montoDe(pool.vacio)))}
      </div>
    `;
  }

  function computeFacturaPendiente(universo) {
    // Alerta: NV con el 100% de sus ítems entregados (Cum_INV completo), con
    // guía emitida, pero SIN factura — el producto salió pero falta
    // facturar. Requiere el fix de "Sin datos" en dataLoader.js para leer
    // guía/factura correctamente.
    const esCompleto = (nv) => nv.items.length > 0 && nv.items.every((it) => (it.inFull || 0) >= 0.999);
    return universo.filter((nv) => esCompleto(nv) && nv.guia && !nv.factura);
  }

  function renderAlertaFacturacion(universo) {
    const pendientes = computeFacturaPendiente(universo);
    const cont = document.getElementById('np-alertas');
    if (pendientes.length === 0) {
      cont.innerHTML = '';
      return;
    }
    const monto = pendientes.reduce((s, nv) => s + (nv.montoTotal || 0), 0);
    cont.innerHTML = `
      <div class="np-alerta-banner">
        <span class="status-badge status-badge--warning">⚠ Falta facturar</span>
        <span><strong>${pendientes.length}</strong> NV entregadas al 100% con guía pero sin factura asociada · ${fmtMoney(monto)}</span>
        <button id="np-alerta-facturar-toggle" class="btn-export">▾ Ver detalle</button>
        <button id="np-alerta-facturar-export" class="btn-export">⬇ Exportar Excel</button>
      </div>
      <div id="np-alerta-facturar-tabla"></div>
    `;

    document.getElementById('np-alerta-facturar-toggle')?.addEventListener('click', (e) => {
      const tablaEl = document.getElementById('np-alerta-facturar-tabla');
      const abierta = tablaEl.innerHTML !== '';
      if (abierta) {
        tablaEl.innerHTML = '';
        e.currentTarget.textContent = '▾ Ver detalle';
        return;
      }
      e.currentTarget.textContent = '▴ Ocultar detalle';
      tablaEl.innerHTML = tableHtml(
        ['NV', 'Cliente', 'Canal', 'Guía', 'Monto', 'Fecha compromiso'],
        pendientes.map((nv) => [nv.nvNumero, nv.cliente, nv.canal || '', nv.guia, fmtMoney(nv.montoTotal), nv.fechaCompromiso ? nv.fechaCompromiso.toLocaleDateString('es-CL') : 'N/A'])
      );
    });

    document.getElementById('np-alerta-facturar-export')?.addEventListener('click', (e) => {
      downloadXlsx(
        `falta_facturar_${new Date().toISOString().slice(0, 10)}.xlsx`,
  ...
  function renderChartPorDia(universo) {
    const map = new Map();
    universo.forEach((nv) => {
      if (!nv.fecCreacion) return;
      const key = startOfDay(nv.fecCreacion).getTime();
      map.set(key, (map.get(key) || 0) + 1);
    });
    const entries = [...map.entries()].sort((a, b) => a[0] - b[0]);
    const box = document.getElementById('np-chart-dia-box');
    if (entries.length === 0) {
      if (box) box.innerHTML = '<p class="kpi-empty">Sin NV creadas en el período filtrado.</p>';
      return;
    }
    if (box && !document.getElementById('chart-np-por-dia')) box.innerHTML = '<canvas id="chart-np-por-dia"></canvas>';
    drawChart('chart-np-por-dia', {
      type: 'bar',
      data: {
        labels: entries.map(([t]) => new Date(t).toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit' })),
        datasets: [{ label: 'NV creadas', data: entries.map(([, c]) => c), backgroundColor: PALETTE.primary, borderRadius: 4, maxBarThickness: 36 }],
      },
      options: {
  ...
  function renderTabla(universo) {
    ...
    if (filas.length === 0) {
      document.getElementById('np-tabla').innerHTML = `
        <div class="kpi-empty">No se encontraron notas de venta con estos filtros. Prueba ampliando el período o limpiando los filtros.</div>
      `;
      nvSeleccionada = null;
      renderDetalleNv(filas);
      return;
    }

    const filasVisibles = filas.slice(0, 300);
    const filasHtml = filasVisibles
      .map(
        (nv) => `
      <tr class="kpi-table-row-clickable" data-nv="${nv.nvNumero}">
        <td>${nv.nvNumero}</td>
        <td>${nv.cliente}</td>
        <td>${nv.canal || ''}</td>
        <td>${statusBadgeHtml(nv)}</td>
        <td>${fmtMoney(nv.montoTotal)}</td>
        <td>${nv.fechaCompromiso ? nv.fechaCompromiso.toLocaleDateString('es-CL') : 'N/A'}</td>
      </tr>`
      )
      .join('');
    ...
    document.getElementById('np-tabla').innerHTML = `
      <div class="np-tabla-acciones">
        <span class="np-resultados-contador"><strong>${filas.length}</strong> notas de venta encontradas${filas.length > 300 ? ' (mostrando las primeras 300)' : ''}</span>
        <button id="np-toggle-tabla" class="btn-export">${tablaAbierta ? '▴ Ocultar tabla' : '▾ Ver tabla'}</button>
        <button id="np-export-tabla" class="btn-export btn-export--primary">⬇ Exportar Excel</button>
      </div>
      ${
        tablaAbierta
          ? `<div class="kpi-table-wrap"><table class="kpi-table">
              <thead><tr><th>NV</th><th>Cliente</th><th>Canal</th><th>Estado</th><th>Monto</th><th>Fecha compromiso</th></tr></thead>
              <tbody>${filasHtml}</tbody>
            </table></div>
            <p class="kpi-note" style="margin-top:8px;">Clic en una fila para ver el detalle completo (vendedor, bloqueado, entrega, atraso, status).</p>`
          : ''
      }
    `;
    ...
    document.getElementById('np-export-tabla')?.addEventListener('click', (e) => {
      downloadXlsx(
        `notas_pendientes_${new Date().toISOString().slice(0, 10)}.xlsx`,
        ['Fecha compromiso', 'NV', 'Cliente', 'Vendedor', 'Canal', 'Bloqueado', 'Entrega', 'Fecha creacion', 'Dias de atraso habiles', 'Status', 'Monto'],
        filas.map((nv) => [
          nv.fechaCompromiso ? nv.fechaCompromiso.toLocaleDateString('es-CL') : '',
          nv.nvNumero,
          nv.cliente,
    ...
    if (tablaAbierta) {
      document.querySelectorAll('#np-tabla tr[data-nv]').forEach((tr) => {
        tr.addEventListener('click', () => {
          nvSeleccionada = tr.dataset.nv === nvSeleccionada ? null : tr.dataset.nv;
          renderDetalleNv(filas);
        });
      });
    }
  ...
  function renderDetalleNv(filas) {
    const panel = document.getElementById('np-tabla-detalle-panel');
    if (!panel) return;
    if (!nvSeleccionada) {
      panel.innerHTML = '';
      return;
    }
    const nv = filas.find((n) => n.nvNumero === nvSeleccionada);
    if (!nv) {
      panel.innerHTML = '';
      return;
    }
    panel.innerHTML = `
      <div class="nv-detail-card">
        <button class="nv-detail-close" id="np-detalle-nv-close">✕</button>
        <div class="nv-detail-grid">
          ...
          <div><strong>Canal:</strong> ${nv.canal || ''}</div>
          <div><strong>Bloqueado:</strong> ${nv.bloqueado ? 'Sí' : 'No'}</div>
          <div><strong>Entrega:</strong> ${nv.entrega || ''}</div>
          <div><strong>Monto:</strong> ${fmtMoney(nv.montoTotal)}</div>
          <div><strong>Fecha creación:</strong> ${nv.fecCreacion ? nv.fecCreacion.toLocaleDateString('es-CL') : 'N/A'}</div>
          <div><strong>Fecha compromiso:</strong> ${nv.fechaCompromiso ? nv.fechaCompromiso.toLocaleDateString('es-CL') : 'N/A'}</div>
          <div><strong>Días de atraso hábiles:</strong> ${Math.round(nv.diasAtrasoHabiles * 10) / 10}</div>
          <div><strong>Status:</strong> ${statusBadgeHtml(nv)}</div>
        </div>
      </div>
    `;
    document.getElementById('np-detalle-nv-close')?.addEventListener('click', () => {
      nvSeleccionada = null;
      renderDetalleNv(filas);
    });
  }
  ...
  function pieCardHtml(id) {
    return `
      <div class="np-pie-card np-pie-card--${id}">
        <div class="np-pie-card-top">
          <div class="np-pie-chart-col">
            <h4>${NP_TITULOS[id]}</h4>
            <div class="chart-box np-pie-chart"><canvas id="chart-np-${id}"></canvas></div>
          </div>
          <div class="np-pie-explain">${NP_EXPLICACIONES[id]}</div>
        </div>
      ...
      drawChart(`chart-np-${id}`, {
        ...
        options: {
          plugins: {
            ...
            // Números visibles siempre sobre la torta, no solo al pasar el
            // mouse. Se ocultan solo en porciones muy chicas (<5%) para que
            // no se amontonen encima de una rebanada angosta.
            datalabels: {
              display: (ctx) => {
                const i = ctx.dataIndex;
                return g.totalCount > 0 && g.counts[i] / g.totalCount >= 0.05;
              },
              color: '#fff',
  ...
  function renderDetallePanel(pool) {
    ...
    abiertos.forEach((id) => {
      document.getElementById(`np-export-${id}`)?.addEventListener('click', (e) => {
        downloadXlsx(
  ...
  function renderAll() {
    ...
    renderAlertaFacturacion(universo);
    renderChartPorDia(universo);
    renderTabla(universo);
    renderPies(pool);
  }
  ...
  function limpiarFiltros() {
    bloqueadoActivo = 'todos';
    entregaActiva = 'todos';
    fechaModo = 'semana';
    searchTerm = '';
    nvSeleccionada = null;
    document.querySelectorAll('#np-filtro-bloqueado .btn-toggle, #np-filtro-entrega .btn-toggle, #np-filtro-fecha .btn-toggle').forEach((b) => {
