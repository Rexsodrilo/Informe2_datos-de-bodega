...
function renderNotasPendientes(el, nvRecords, skuInfo) {
  ...
  // Detalle por NV con quiebre: productos afectados y su déficit (a nivel de
  // SKU, ver computeQuiebreStock), para la zona de detalle compartida.
    ...
    return conQuiebre.map((nv) => {
      const productosTexto = nv.items
        ...
        .map((it) => {
          ...
          const faltan = info ? Math.max(0, (info.demandaVigente || 0) - (info.stockSistema || 0)) : 0;
          return `${it.detProd || it.codProd || '(sin código)'} (faltan ${faltan})`;
        })
        .join(' | ');
