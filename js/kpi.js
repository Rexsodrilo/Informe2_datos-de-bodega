...
export function computeKpis(nvRecords, contHodo, range) {
  ...
  const noDespachadasAtrasadas = nvRecords
    ...
    .map((nv) => ({
      ...nv,
      diasAtrasoHabiles: businessHoursElapsed(nv.fechaCompromiso, ahora) / horasHabilesPorDia,
    }))
  ...
  // Bloque A2: histórico de atrasos por cliente y trimestre — mismo
  // universo que el Bloque A (NV sin despachar que incumplen las 48h),
  // agrupado por trimestre de la fecha de CREACIÓN (no de compromiso).
  const trimestreDe = (fecha) => `${fecha.getFullYear()}-T${Math.floor(fecha.getMonth() / 3) + 1}`;

  const clienteTrimestreMap = new Map(); // cliente -> Map(trimestre -> { count, sumDias })
  noDespachadasAtrasadas.forEach((nv) => {
    if (!nv.fecCreacion) return;
    const trimestre = trimestreDe(nv.fecCreacion);
    if (!clienteTrimestreMap.has(nv.cliente)) clienteTrimestreMap.set(nv.cliente, new Map());
    const porTrimestre = clienteTrimestreMap.get(nv.cliente);
    if (!porTrimestre.has(trimestre)) porTrimestre.set(trimestre, { count: 0, sumDias: 0 });
    const g = porTrimestre.get(trimestre);
    g.count += 1;
    g.sumDias += nv.diasAtrasoHabiles;
  });

  const topClientesAtraso = [...clienteTrimestreMap.entries()]
    .map(([cliente, m]) => ({ cliente, total: [...m.values()].reduce((s, g) => s + g.count, 0) }))
    .sort((a, b) => b.total - a.total)
    .slice(0, 10)
    .map((c) => c.cliente);

  const trimestresAtraso = [
    ...new Set(noDespachadasAtrasadas.filter((nv) => nv.fecCreacion).map((nv) => trimestreDe(nv.fecCreacion))),
  ].sort();

  const historicoAtrasoPorCliente = {
    trimestres: trimestresAtraso,
    detalle: topClientesAtraso.map((cliente) => ({
      cliente,
      porTrimestre: trimestresAtraso.map((trimestre) => {
        const g = clienteTrimestreMap.get(cliente)?.get(trimestre);
        return { trimestre, count: g ? g.count : 0, diasPromedio: g ? g.sumDias / g.count : 0 };
      }),
    })),
  };
  ...
  return {
    alertas: { noDespachadasAtrasadas, despachadoPorCanal, historicoAtrasoPorCliente },
...
function renderAlertas(el, data) {
  ...
  const canales = distinct(data.noDespachadasAtrasadas.map((nv) => nv.canal));
  const vendedores = distinct(data.noDespachadasAtrasadas.map((nv) => nv.vendedor));
  const filterOptions = (values) =>
    '<option value="todos">Todos</option>' + values.map((v) => `<option value="${v}">${v}</option>`).join('');
  const canalButtons = (values) =>
    ['todos', ...values]
      .map(
        (v) =>
          `<button class="btn-toggle${v === 'todos' ? ' active' : ''}" data-value="${v}">${v === 'todos' ? 'Todos' : v}</button>`
      )
      .join('');

  el.innerHTML = `
    <div class="kpi-alertas-header">
      <div>
      ...
      <div class="kpi-alertas-filters">
        <div class="kpi-filter-group">
          <span class="kpi-filter-label">Canal</span>
          <div class="kpi-filter-buttons" id="alertas-filtro-canal">${canalButtons(canales)}</div>
        </div>
        <div class="kpi-filter-group">
          <span class="kpi-filter-label">Vendedor</span>
          <select id="alertas-filtro-vendedor" class="kpi-filter-select">${filterOptions(vendedores)}</select>
        </div>
      </div>
    </div>
    <div id="alertas-bloque-a"></div>

    <h3>Histórico de atrasos por cliente (por trimestre de creación)</h3>
    <p class="kpi-note">Top 10 clientes con más NV atrasadas en total, agrupadas por el trimestre en que se creó cada NV (no depende del selector de período).</p>
    <div class="chart-box" style="max-width:100%;"><canvas id="chart-atraso-trimestre"></canvas></div>
    ${tableHtml(
      ['Cliente', 'Trimestre', 'N° NV atrasadas', 'Días de atraso (promedio)'],
      data.historicoAtrasoPorCliente.detalle.flatMap((c) =>
        c.porTrimestre
          .filter((t) => t.count > 0)
          .map((t) => [c.cliente, t.trimestre, t.count, Math.round(t.diasPromedio * 10) / 10])
      )
    )}
    ...
    ${
      data.despachadoPorCanal.length === 0
        ? '<p class="kpi-empty">Sin despachos registrados en este período. Prueba con Semana o Mes, o elige otra fecha.</p>'
  ...
  drawChart('chart-atraso-trimestre', {
    type: 'bar',
    data: {
      labels: data.historicoAtrasoPorCliente.trimestres,
      datasets: data.historicoAtrasoPorCliente.detalle.map((c, i) => ({
        label: c.cliente,
        data: c.porTrimestre.map((t) => t.count),
  ...
  // Bloque A se re-renderiza aparte al cambiar los filtros de Canal/Vendedor,
  // sin tocar el histórico ni "Despachado por canal" (que siguen mostrando
  // el universo completo, según lo acordado).
  let filtradas = data.noDespachadasAtrasadas;
  let canalActivo = 'todos';

  function renderBloqueA() {
    const vendedor = document.getElementById('alertas-filtro-vendedor').value;

    filtradas = data.noDespachadasAtrasadas.filter(
      (nv) => (canalActivo === 'todos' || nv.canal === canalActivo) && (vendedor === 'todos' || nv.vendedor === vendedor)
    );

    const total = filtradas.length;
    const monto = filtradas.reduce((s, nv) => s + nv.montoTotal, 0);

    document.getElementById('alertas-bloque-a').innerHTML = `
      <div class="kpi-card-row">
        ${kpiCard('NV sin despachar y atrasadas', total, fmtMoney(monto) + ' en riesgo')}
      </div>
      <h3>NV que incumplen las 48 horas hábiles</h3>
      <button id="export-alertas" class="btn-export">⬇️ Exportar CSV</button>
      ${tableHtml(
        ['NV', 'Cliente', 'Fecha<br>Creación', 'Fecha<br>Compromiso', 'Días de atraso hábiles'],
        filtradas.slice(0, 200).map((nv) => [
          nv.nvNumero,
          nv.cliente,
    ...
    document.getElementById('export-alertas')?.addEventListener('click', () => {
      downloadCsv(
        ...
        ['NV', 'Cliente', 'Vendedor', 'Canal', 'Tipo', 'Fecha creacion', 'Fecha compromiso', 'Dias de atraso habiles', 'Monto'],
        filtradas.map((nv) => [
          nv.nvNumero,
          nv.cliente,
          nv.vendedor,
  ...
  renderBloqueA();
  document.getElementById('alertas-filtro-vendedor')?.addEventListener('change', renderBloqueA);
  document.querySelectorAll('#alertas-filtro-canal .btn-toggle').forEach((btn) => {
    btn.addEventListener('click', () => {
      canalActivo = btn.dataset.value;
      document.querySelectorAll('#alertas-filtro-canal .btn-toggle').forEach((b) => b.classList.remove('active'));
