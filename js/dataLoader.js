...
/** Busca el nombre real de una hoja tolerando espacios extra/mayúsculas
 * distintas (ej. la hoja "INV sistema " viene con un espacio final que no es
 * consistente entre archivos). */
function findSheetName(workbook, target) {
  const normTarget = normalizeKey(target);
  return workbook.SheetNames.find((name) => normalizeKey(name) === normTarget) || target;
}
...
/** Normaliza un código de producto para poder cruzarlo entre hojas aunque
 * una lo guarde como texto ("5000220") y la otra como número (5000220). */
...
const ALIASES = {
  ...
  inFull: ['Cum_INV', 'cum inv', 'cumplimiento inventario', 'in full', 'infull'],
};

const CORTE_SIGNATURE = ['nvestado', 'nvnumero', 'canal', 'nvtotlinea'];

// ---------------------------------------------------------------------------
// INV sistema (stock actual por SKU según el sistema — hoja aparte del
// mismo libro, la fuente real y siempre actualizada: "Stock_Dis" en Corte
// viene de aquí mismo pero solo se completa en una fracción de las líneas).
// ---------------------------------------------------------------------------

const INV_SISTEMA_ALIASES = {
  sku: ['Cod. Producto', 'codigo producto', 'cod producto'],
  invSis: ['INV Sis', 'inv sis'],
};

const INV_SISTEMA_SIGNATURE = ['cod producto', 'inv sis'];

/** Mapa SKU normalizado -> stock actual según el sistema. */
function parseInvSistema(workbook) {
  const sheetName = findSheetName(workbook, 'INV sistema');
  const { rows } = sheetToRows(workbook, sheetName, INV_SISTEMA_SIGNATURE);
  const map = new Map();
  rows.forEach((row) => {
    const sku = normalizeSku(getField(row, INV_SISTEMA_ALIASES.sku));
    if (!sku) return;
    map.set(sku, toNumber(getField(row, INV_SISTEMA_ALIASES.invSis)));
  });
  return map;
}

function parseCorte(workbook, invSistemaMap) {
  const { rows } = sheetToRows(workbook, 'Corte', CORTE_SIGNATURE);
  const groups = new Map();
  // Demanda total (Σ nvCant) por SKU, sumando SOLO las NV vigentes (estado
  // A) — varias NV pueden pedir el mismo producto a la vez, así que el
  // quiebre real se decide por esta suma contra el stock del sistema, no
  // por una sola línea contra su propio "Stock_Dis" (que además viene vacío
  // la mayoría de las veces).
  const demandaVigentePorSku = new Map();
  ...
  rows.forEach((row) => {
    ...
    if (!groups.has(nvNumero)) {
      groups.set(nvNumero, {
        ...
        trans: emptyIfZero(getField(row, ALIASES.trans)),
        auditor: emptyIfZero(getField(row, ALIASES.auditor)),
    ...
    const precioUnitario = toNumber(getField(row, ALIASES.nvPrecio));
    const codProd = getField(row, ALIASES.codProd).toString().trim();

    if (nvEstado === 'A') {
      const skuKey = normalizeSku(codProd);
      if (skuKey) {
        const acc = demandaVigentePorSku.get(skuKey) || { cant: 0, precioSum: 0, precioCount: 0 };
        acc.cant += cant;
        if (precioUnitario > 0) {
          acc.precioSum += precioUnitario;
          acc.precioCount += 1;
        }
        demandaVigentePorSku.set(skuKey, acc);
      }
    }
    ...
    g.items.push({
      ...
      montoLinea,
      inFull,
      quiebre: false, // se calcula abajo, tras sumar la demanda total por SKU
    });
  });

  // Quiebre real: por SKU, la demanda TOTAL de todas las NV vigentes que
  // piden ese producto contra el stock que informa "INV sistema" — no la
  // cantidad de una sola línea contra su propio Stock_Dis. Si el SKU no
  // aparece en "INV sistema" no hay forma de confirmar el quiebre, así que
  // se asume que no lo hay (no se alarma sin dato real).
  //
  // skuInfo guarda demanda/stock/precio UNA VEZ POR PRODUCTO (no por línea)
  // y solo para los que sí están en quiebre — si se guardara en cada línea
  // (había ~62.000), el data.json publicado quedaba pesando varios MB de
  // más solo por repetir los mismos 2-3 números miles de veces, al punto de
  // superar el límite de 25MB para subir archivos por la web de GitHub.
  const skuInfo = {};
  demandaVigentePorSku.forEach((acc, skuKey) => {
    if (!invSistemaMap.has(skuKey)) return;
    const stockSistema = invSistemaMap.get(skuKey);
    if (acc.cant <= stockSistema) return;
    skuInfo[skuKey] = {
      demandaVigente: acc.cant,
      stockSistema,
      precioPromedio: acc.precioCount > 0 ? acc.precioSum / acc.precioCount : 0,
    };
  });

  groups.forEach((g) => {
    if (g.nvEstado !== 'A') return;
    g.items.forEach((item) => {
      item.quiebre = !!skuInfo[normalizeSku(item.codProd)];
    });
  });

  return { groups, skuInfo };
}
...
function mergeNvData(corteGroups, despByNv) {
  ...
  corteGroups.forEach((c) => {
    ...
    result.push({
      ...
      // Solo el OP_Picking / Auditor de la hoja Corte, sin combinar con
      // Desp — los criterios de las tarjetas de "Notas de ventas vigentes"
      // (en proceso de preparación / en despacho) piden específicamente
      // estos campos de Corte.
      opPickingCorte: c.opPicking,
      auditorCorte: c.auditor,
      auditor: d && d.auditor,
      chofer,
      patente,
      trans: c.trans,
...
export function loadWorkbookFromArrayBuffer(arrayBuffer) {
  ...
  const invSistemaMap = parseInvSistema(workbook);
  const { groups: corteGroups, skuInfo } = parseCorte(workbook, invSistemaMap);
  const despByNv = parseDesp(workbook);
  const nvRecords = mergeNvData(corteGroups, despByNv);
  const contHodo = parseContHodo(workbook);

  return { nvRecords, contHodo, loadedAt: new Date(), skuInfo };
}
