...
function mergeNvData(corteGroups, despByNv) {
  ...
  corteGroups.forEach((c) => {
    ...
    const esRetiro = c.entrega === 'RETIRA';

    // KPI 4: NV bajo $300.000 despachada igual.
    const esBajoMonto = c.montoTotal < 300000 && (c.nvEstado === 'C' || c.statusDes === 'DESPACHADO');

    // KPI 7: Desp marca "DESPACHADO" pero sin chofer/patente asignados ->
    // probable retiro mal registrado como despacho.
    const retiroMalMarcado =
      !!d && d.status === 'DESPACHADO' && !d.chofer && !d.patente;

    // Cumplimiento de compromiso (excluye retiros, que no aplican).
    let cumplimiento = null; // 'A_TIEMPO' | 'ATRASADO' | 'SIN_COORDINAR' | null (n/a)
    if (!esRetiro && fechaCompromiso) {
      if (fCoordinacion) {
