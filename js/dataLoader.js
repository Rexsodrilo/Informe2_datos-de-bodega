...
function mergeNvData(corteGroups, despByNv) {
  ...
  corteGroups.forEach((c) => {
    ...
    result.push({
      ...
      // Solo el OP_Picking de la hoja Desp, SIN caer a Corte — a diferencia
      // de `opPicking` de arriba. Lo usa específicamente el criterio de
      // "En proceso" (picker asignado en Desp + Status de Desp vacío).
      opPickingDesp: d && d.opPicking,
      auditor: d && d.auditor,
      chofer,
      patente,
      trans: c.trans,
