...
function parseCorte(workbook, invSistemaMap) {
  ...
  groups.forEach((g) => {
    ...
    g.items.forEach((item) => {
      const skuKey = normalizeSku(item.codProd);
      if (!skuKey || !invSistemaMap.has(skuKey)) return;
      const demandaVigente = demandaVigentePorSku.get(skuKey) || 0;
      const stockSistema = invSistemaMap.get(skuKey);
      item.demandaVigente = demandaVigente;
      item.stockSistema = stockSistema;
      item.quiebre = demandaVigente > stockSistema;
    });
  });
