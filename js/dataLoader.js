...
function parseCorte(workbook, invSistemaMap) {
  ...
  // quiebre real (¿este producto SÍ o NO alcanza?) se decide por esta suma
  // contra el stock del sistema, no por una sola línea contra su propio
  // "Stock_Dis" (que además viene vacío la mayoría de las veces). Esto solo
  // determina el flag `quiebre` de cada línea — la cantidad/valor que se
  // muestra en el detalle es siempre la de esa línea (nvCant/nvTotLinea),
  // no esta suma.
  ...
  rows.forEach((row) => {
    ...
    if (nvEstado === 'A') {
      ...
      if (skuKey) demandaVigentePorSku.set(skuKey, (demandaVigentePorSku.get(skuKey) || 0) + cant);
    }
  ...
  // se asume que no lo hay (no se alarma sin dato real). Esto solo decide
  // el flag `quiebre` de cada línea; la cantidad/valor que se muestra en
  // pantalla sigue siendo el de esa línea puntual (nvCant/nvTotLinea).
  const skusEnQuiebre = new Set();
  demandaVigentePorSku.forEach((demandaTotal, skuKey) => {
    if (invSistemaMap.has(skuKey) && demandaTotal > invSistemaMap.get(skuKey)) skusEnQuiebre.add(skuKey);
  });

  groups.forEach((g) => {
    if (g.nvEstado !== 'A') return;
    g.items.forEach((item) => {
      item.quiebre = skusEnQuiebre.has(normalizeSku(item.codProd));
    });
  });
...
export function loadWorkbookFromArrayBuffer(arrayBuffer) {
  ...
  const corteGroups = parseCorte(workbook, invSistemaMap);
